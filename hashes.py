...
class Prototype:
    ...
    @staticmethod
    def _repr_format(obj):
        if isinstance(obj, str):
            return f"'{obj}'"
        return repr(obj)

    def __dict__(self):
        return {"regex": self.regex, "modes": self.modes}
...
prototypes = [
    ...
    Prototype(
        ...
        modes=[
            HashInfo(
                name="Wordpress ≥ v2.6.2", hashcat=400, john="phpass", extended=False
            ),
            HashInfo(
                name="Joomla ≥ v2.5.18", hashcat=400, john="phpass", extended=False
            ),
            HashInfo(
                name="PHPass' Portable Hash", hashcat=400, john="phpass", extended=False
            ),
    ...
    Prototype(
        ...
        modes=[
            HashInfo(name="IP.Board ≥ v2+", hashcat=2811, john=None, extended=False)
        ],
    ),
    Prototype(
        regex=re.compile(r"^[a-f0-9]{32}:.{8}$", re.IGNORECASE),
        modes=[HashInfo(name="MyBB ≥ v1.2+", hashcat=2811, john=None, extended=False)],
    ),
    Prototype(
    ...
    Prototype(
        ...
        modes=[
            HashInfo(name="vBulletin ≥ v3.8.5", hashcat=2711, john=None, extended=False)
        ],
    ),
    Prototype(
    ...
    Prototype(
        ...
        modes=[HashInfo(name="SMF ≥ v1.1", hashcat=121, john=None, extended=False)],
    ),
    Prototype(
        regex=re.compile(
    ...
    Prototype(
        ...
        modes=[
            HashInfo(
                name="SHA-1 Crypt", hashcat=15100, john="sha1crypt", extended=False
            )
        ],
    ),
    Prototype(
    ...
    Prototype(
        ...
        modes=[
            HashInfo(name="Android FDE ≤ 4.3", hashcat=8800, john="fde", extended=False)
        ],
    ),
    Prototype(
        regex=re.compile(
    ...
    Prototype(
        ...
        modes=[
            HashInfo(name="BitLocker", hashcat=22100, john="bitlocker", extended=False),
        ],
    ),
    Prototype(
        regex=re.compile(r"\$racf\$\*.{1,}\*[A-F0-9]{16}", re.IGNORECASE),
        modes=[
            HashInfo(name="RACF", hashcat=8500, john=None, extended=False),
        ],
    ),
    Prototype(
        regex=re.compile(
            r"^\$sshng\$4\$16\$[0-9]{32}\$1232\$[a-f0-9]{2464}$", re.IGNORECASE
        ),
        modes=[
            HashInfo(
    ...
    Prototype(
        ...
        modes=[
            HashInfo(
                name="RAR3-p (Uncompressed)", hashcat=23700, john="rar", extended=False
            ),
        ],
    ),
    Prototype(
        ...
        modes=[
            HashInfo(
                name="RAR3-p (Compressed)", hashcat=23800, john="rar", extended=False
            ),
        ],
    ),
    Prototype(
        ...
        modes=[
            HashInfo(name="RAR3-hp", hashcat=12500, john="rar", extended=False),
        ],
    ),
    Prototype(
        regex=re.compile(
        ...
        modes=[
            HashInfo(name="RAR5", hashcat=13000, john="rar5", extended=False),
        ],
    ),
    Prototype(
        regex=re.compile(
